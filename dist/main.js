...
class FileSystem {
    ...
    make_channel() {
        let path = $bXDXC.getUrlPath(`?s=${this._session_num}`);
    ...
    static _tmp_id_to_real(tmp_id, res) {
        ...
        if (FileSystem._files_to_upload[tmp_id] != null && tmp.file != null) {
            ...
            let path = $bXDXC.getUrlPath(`?s=${fs._session_num}&p=${tmp._server_id}`);
            const xhr_object = FileSystem._my_xml_http_request();
...
var $kTWwb = parcelRequire("kTWwb");
function $8b5433c107cc84a4$export$e947614430708c91(searchQuery = '') {
    let path = '';
    if ($kTWwb.FileSystem.CONNECTOR_TYPE === 'Node' || $kTWwb.FileSystem.is_cordova) {
        path = `http://${$kTWwb.FileSystem._url}`;
        if ($kTWwb.FileSystem._port) path += `:${$kTWwb.FileSystem._port}`;
...
class $4b25d2d7c823331f$export$96d7e0bc5363b2c6 extends $laC9O.Model {
    ...
    load(callback) {
        ...
        else (_a = $kTWwb.FileSystem.get_inst()) === null || _a === void 0 || _a.load_ptr(this.data.value, callback);
    }
    _get_fs_data(out) {
        $kTWwb.FileSystem.set_server_id_if_necessary(out, this);
...
$parcel$export($2ab6e48b91659bf6$exports, "spinalCore", () => $2ab6e48b91659bf6$export$a34888876ba95657);
...
(function($2ab6e48b91659bf6$export$a34888876ba95657) {
    ...
    function connect(options) {
        ...
        $kTWwb.FileSystem._home_dir = parsedOpt.pathname;
        $kTWwb.FileSystem._url = parsedOpt.hostname;
        $kTWwb.FileSystem._port = parsedOpt.port;
        if (parsedOpt.username) {
            $kTWwb.FileSystem._userid = parsedOpt.username;
            if (parsedOpt.password) $kTWwb.FileSystem._password = parsedOpt.password;
        } else {
            // set default user id
            $kTWwb.FileSystem._userid = 644;
            $kTWwb.FileSystem._password = '';
        }
        return new $kTWwb.FileSystem();
    }
    ...
    function store(fs, model, path, callback_success, callback_error) {
        ...
        return fs.load_or_make_dir($kTWwb.FileSystem._home_dir + path, function(dir, err) {
    ...
    $2ab6e48b91659bf6$export$a34888876ba95657.register_models = $kpG8j.ModelProcessManager.register_models;
    ...
    function load(fs, path, callback_success, callback_error) {
        ...
        if (lst[0] === '') lst.splice(0, 1);
        path = lst.join('/'); // Absolute paths are not allowed
        return fs.load_or_make_dir(`${$kTWwb.FileSystem._home_dir}${path}`, (current_dir, err1)=>{
...
$2807f303cd754282$export$a4e9f07232169aad($2ab6e48b91659bf6$export$a34888876ba95657, 'spinalCore');
